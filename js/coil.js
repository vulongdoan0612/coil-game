...
var Coil = (function () {
  ...
  function initialize() {
    ...
    if (canvas && canvas.getContext) {
      ...
      lagWarningAction.addEventListener(
        "click",
        onLagWarningButtonClick,
        false
      );
      ...
      alert(
        "Doesn't seem like your browser supports the HTML5 canvas element :("
      );
    }
  }
  ...
  function activate3dEffects() {
    ...
    effectsVertices = new Float32Array([
    ...
    effectsTexture = WebGLUtil.loadTexture(
      context3d,
      "images/texture.png",
      $.proxy(function () {
        ...
        if (
          ...
          var t0 = context3d.getUniformLocation(
            effectsShaderProgram,
            "texture"
          );
    ...
    while (candidates.length) {
      ...
      while (i--) {
        if (
          candidate &&
          intersections[i] &&
          candidate[0] === intersections[i][0] &&
          candidate[1] === intersections[i][1]
        ) {
          candidate = null;
        }
      }
        ...
        while (j--) {
          ...
          if (pixels[index + 1] === 255 && pixels[index + 2] === 255) {
            if (
              ...
              enemy.type === ENEMY_TYPE_BOMB_MOVER
            ) {
  ...
  function updateMeta() {
    ...
    if (timeThisFrame > timeLastSecond + 1000) {
      ...
      fps = Math.min(
        Math.round(
          (framesThisSecond * 1000) / (timeThisFrame - timeLastSecond)
        ),
        FRAMERATE
      );
    ...
    if (
      frameCount > FRAMERATE * 6 &&
      Math.round((fpsMin + fpsMax + fps) / 3) < 30
    ) {
      showLagWarning();
    }
  }
  ...
  function updatePlayer() {
    ...
    if (player.energy === 0) {
      if (matchId !== "") {
        client.match
          .completeMatch(matchId, { score: Number(score.toFixed()) })
          .then((data) => {
            scorePanel.style.display = "block";
            scorePanel.querySelector("p").innerHTML = Math.floor(score);
  ...
  function updateEnemies() {
    ...
    var canAddMovers =
      numberOfMovers / enemies.length < 0.3 &&
      frameCount > ENEMY_MOVER_START_FRAME;
    ...
    while (i-- && Math.random() > 0.85) {
      ...
      enemy = new Enemy();
      enemy.x =
        padding + Math.round(Math.random() * (world.width - padding - padding));
      enemy.y =
        padding +
        Math.round(Math.random() * (world.height - padding - padding));
      enemy.type = type;
    ...
    while (i--) {
      ...
      if (
        enemy.type === ENEMY_TYPE_BOMB_MOVER ||
        enemy.type === ENEMY_TYPE_NORMAL_MOVER
      ) {
  ...
  function updateEffects() {
    ...
    while (i--) {
      ...
      if (effect.alive) {
        effect.time = Math.min(
          (effect.time + 0.01) * (1 + (1 - effect.time)),
          1
        );
      } else {
  ...
  function renderNotifications() {
    ...
    while (i--) {
      ...
      context.fillText(
        p.text,
        p.x - context.measureText(p.text).width * 0.5,
        p.y + 4 * p.scale
      );
      context.restore();
  ...
  function renderHeader() {
    ...
    context.fillRect(
      0,
      2,
      (player.animatedEnergy / 100) * energyBarWidth,
      energyBarHeight
    );
    context.restore();
  ...
  function onCanvasTouchStartHandler(event) {
    if (event.touches.length == 1) {
      ...
      mouse.x =
        event.touches[0].pageX - (window.innerWidth - world.width) * 0.5;
      mouse.y =
  ...
  function onCanvasTouchMoveHandler(event) {
    if (event.touches.length == 1) {
      ...
      mouse.x =
        event.touches[0].pageX - (window.innerWidth - world.width) * 0.5;
      mouse.y =
        event.touches[0].pageY - (window.innerHeight - world.height) * 0.5 - 20;
    }
  }
